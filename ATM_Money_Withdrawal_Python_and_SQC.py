...
pandy = psycopg2.connect(host = 'database-1.cmyigtchqfm2.eu-north-1.rds.amazonaws.com',user = 'postgres', password='Pandy##1010',port = 5432,database ='Pandy_localDB')
kumar = pandy.cursor()
...
if len(Acc_no)>0:
    ...
    kumar.execute("insert into audit(Account_Number,Withdraw_Amount,Withdraw_Date_Time) values(%s,%s,%s)",[AccountNumber,Amount,Time])
    kumar.execute("update Audit AI set Location=Bank_details.Location,Balance=Bank_details.Balance from Bank_details where Bank_Details.Account_number=AI.Account_number")
    pandy.commit()
    #kumar.execute("date: $CURRENT_YEAR-$CURRENT_MONTH-$CURRENT_DATE $CURRENT_HOUR:$CURRENT_MINUTE:$CURRENT_SECOND")
#kumar.execute("select * from Audit")
#pandy.commit()